package com.nazirhakim.colorsmatch;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        runnable = new Runnable() {
            ...
            public void run() {
                ...
                //check if there is still some time left in the progressbat
                if (currentTime > 0){
                    handler.postDelayed(runnable, 100);
                } else {
                    // check if the color of the arrow and the button are the same
                    if(buttonState == arrowState){
                        //increase points and show them
                        currentPoints = currentPoints + 1;
                        tv_points.setText("Points: " + currentPoints);

                        //make the speed higher after every turn / if the speed is 1 second make it again 2 second
    ...
    //display the arrow color according to the generate number
